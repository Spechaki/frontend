...
export default function ServiceTrainerInfo({ trainer }) {
  if (!trainer) return null;

  const fullName = `${trainer.first_name} ${trainer.last_name}`;
  const trainerImage = trainer.profile_picture
    ...
    <section>
      ...
      <div className="flex items-center bg-sky-100 p-4 rounded gap-4">
        ...
        <div>
          ...
          <Rating value={trainer.average_rating ?? 0} precision={0.5} readOnly />
        </div>
      </div>
    </section>
  );
